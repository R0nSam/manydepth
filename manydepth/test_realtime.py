...
def load_and_preprocess_image(image, resize_width, resize_height): #(image_path, resize_width, resize_height): # RON changed here
...
def test_simple(args):
    ...
    with torch.no_grad():
        ...
        while True:
            ...
            source_image = pil.fromarray(video.read())           # array conversion required for next line to work
            #source_image = cv2.cvtColor(source_image, cv2.COLOR_BGR2RGB)     # BGR to RGB conversion
            source_image, _ = load_and_preprocess_image(source_image,
                                                resize_width=encoder_dict['width'],
                                                resize_height=encoder_dict['height'])
            
            input_image = pil.fromarray(video.read())
            #input_image = cv2.cvtColor(input_image, cv2.COLOR_BGR2RGB)
            input_image, original_size = load_and_preprocess_image(input_image,
                                                        resize_width=encoder_dict['width'],
                                                        resize_height=encoder_dict['height']) 
            ...
            for plot_name, toplot in (('costvol_min', lowest_cost), ('disp', sigmoid_output_resized)):
                ...
                cv2.imshow("ManyDepth (Press escape key to cancel)", colormapped_im)
